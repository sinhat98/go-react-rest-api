...
func (tc *taskController) GetAllTasks(c echo.Context) error {
	...
	// JWTトークンからユーザー情報を取得
	// JWTトークンを使用して、リクエストを送信したユーザーを特定する
	// これにより、ユーザーが自分のタスクのみにアクセスできるようになる
	user := c.Get("user").(*jwt.Token)

	// トークンからクレーム(ペイロード)を取得
	// JWTトークンのペイロード部分には、ユーザーに関する情報が格納されている
	claims := user.Claims.(jwt.MapClaims)

	// クレームからユーザーIDを取得
	// ペイロードからユーザーIDを取り出し、このIDを使って
	// データベースから該当ユーザーのタスクのみを取得する
